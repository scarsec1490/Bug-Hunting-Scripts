...
for ((i=1;i<=DEPTH;i++)); do
    ...
    if [[ $i -eq 1 ]]; then
        ...
        # Subsequent rounds: previous round's output
        if [[ -s "$PREV_FILE" ]]; then
    ...
    # Stop if 0 subs found
    if [[ $NEW_SUBS -eq 0 ]]; then
        echo -e "${YELLOW}[!] No subdomains found in round $i. Stopping subdomain discovery early.${NC}"
        rm -f "$CURRENT_FILE"
        break
    fi
...
# Merge all rounds into allsubs.txt
echo -e "${GREEN}[+] Merging all rounds -> allsubs.txt${NC}"
...
# Run gau and waybackurls in parallel on all subs
if [[ -s "$OUTPUT_DIR/allsubs.txt" ]]; then
...
# Merge all URLs into final allurls.txt
