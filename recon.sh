...
#   ./recon.sh scope.txt
#   ./recon.sh live_subs.txt --crawl   (skip subdomain discovery, go straight to crawling)
...
# ----------------- Input Validation -----------------
if [ -z "$1" ]; then
    echo -e "${RED}[-] Usage: $0 <domain | domains.txt> [--crawl]${NC}"
    exit 1
fi
...
# ============================================================
# CRAWL ONLY MODE — Skip subdomain discovery
# ============================================================
if [[ "$CRAWL_ONLY" == true ]]; then

    echo -e "${YELLOW}======================================${NC}"
    echo -e "${CYAN}[+] Mode: CRAWL ONLY (--crawl flag detected)${NC}"
    echo -e "${YELLOW}[+] Target: $TARGET${NC}"
    echo -e "${YELLOW}[+] Output directory: $OUTPUT_DIR/${NC}"
    echo -e "${YELLOW}======================================${NC}"

    # Use provided file or single domain as the crawl input
    if [[ -f "$TARGET" ]]; then
        CRAWL_INPUT="$TARGET"
        echo -e "${GREEN}[+] Using $TARGET as crawl input ($(check_and_count "$TARGET") hosts)${NC}"
    else
        # Single domain given — write it to a temp file
        CRAWL_INPUT="$OUTPUT_DIR/crawl_input.txt"
        echo "$TARGET" > "$CRAWL_INPUT"
        echo -e "${GREEN}[+] Using $TARGET as crawl input${NC}"
    fi
    ...
    # Katana
    if [[ -s "$CRAWL_INPUT" ]]; then
        katana -list "$CRAWL_INPUT" -d 5 -o "$OUTPUT_DIR/katana_urls.txt"
        echo -e "${YELLOW}[+] Katana URLs: $(check_and_count "$OUTPUT_DIR/katana_urls.txt")${NC}"
    else
        touch "$OUTPUT_DIR/katana_urls.txt"
        echo -e "${YELLOW}[!] No hosts to scan with katana.${NC}"
    fi

    # GAU + Waybackurls in parallel
    if [[ -s "$CRAWL_INPUT" ]]; then
        cat "$CRAWL_INPUT" | gau > "$OUTPUT_DIR/gau_urls.txt" 2>/dev/null &
        gau_pid=$!
        cat "$CRAWL_INPUT" | waybackurls > "$OUTPUT_DIR/wayback_urls.txt" 2>/dev/null &
        wayback_pid=$!
        wait $gau_pid
        wait $wayback_pid
    ...
    # ----------------- Summary -----------------
    echo -e "${YELLOW}======================================${NC}"
    echo -e "${GREEN}[+] Crawl completed. Files generated in $OUTPUT_DIR/:${NC}"
    for f in katana_urls.txt gau_urls.txt wayback_urls.txt allurls.txt; do
...
# ============================================================
# FULL RECON MODE — Subdomain discovery + Crawling
# ============================================================
...
echo -e "${YELLOW}======================================${NC}"
echo -e "${CYAN}[+] Mode: FULL RECON${NC}"
echo -e "${YELLOW}[+] Target: $TARGET${NC}"
